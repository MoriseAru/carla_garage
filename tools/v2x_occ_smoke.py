...
cfg = cfgmod.GlobalConfig(); cfg.initialize(root_dir=[a.root], setting="all", use_v2x=1, use_v2x_aux=1, v2x_occ_weight=4.0, v2x_rate_dropout=1, v2x_p_zero=0.1)
check("loss_hidden_hazard" in cfg.detailed_loss_weights, "config has loss_hidden_hazard weight")
...
check(hasattr(net, "hidden_hazard_head"), "aux head built")
...
def run(states, mask, weight, aux_label, aux_mask):
    ...
    return net.compute_loss(pred_wp=out[0], pred_target_speed=out[1], pred_checkpoint=out[2], pred_semantic=out[3], pred_bev_semantic=out[4], pred_depth=out[5],
                            ...
                            sample_weight=weight, aux_label=aux_label, aux_mask=aux_mask, **labels), out
with torch.no_grad():
    net.eval()
    l_plain, _ = run(st, mk, None, None, None)
    l_w1, _ = run(st, mk, torch.ones(bs, device=dev), None, None)
...
check(len(g) == 4, f"grad reached aux head params {len(g)}/4"); check(all(k in cfg.detailed_loss_weights for k in l_c), "every loss key has a weight in config (train loop would KeyError otherwise)")
# ---- (5) config round trip ----
c2 = jsonpickle.decode(jsonpickle.encode(cfg)); check(getattr(c2, "use_v2x_aux", None) == 1 and getattr(c2, "v2x_occ_weight", None) == 4.0 and getattr(c2, "v2x_p_zero", None) == 0.1, "config.json round-trip keeps scheme-C fields")
